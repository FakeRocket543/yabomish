...
- **全新圖示（摺紙蝦）＋狀態列簡化** — App 圖示改為摺紙風格的捲曲蝦（macOS／iOS 同款，FLUX 生成）；狀態列／輸入法清單改顯示**文字**（移除 `tsInputMethodIconFileKey`），切換顯示收斂為 **Yabo（預設）／Yabomish** 二擇（移除繁中／🦐，舊值自動遷移）。**蝦頭方向功能移除**（狀態列改文字顯示後無作用；含設定頁區塊、IconApply、偏好鍵）。注意：名稱變更需重新安裝或移除重加輸入法才會刷新 TIS 快取
