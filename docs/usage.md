...
- 用數字鍵選擇同音字
- 若開啟「**同音字自動退出**」：選字後自動退出同音字模式
- 若關閉：選字後仍留在同音字模式，再打 `,,TO` + `Space` 退出
...
### 查字歷史
- 注音反查／同音字／拼音查碼**選字送出時自動記錄**（存在本機 freq.db，上限 1000 筆）
- `,,LH` + `Space` → 列出最近 20 筆（新→舊），格式：`字 碼 ←查詢`
- `,,RH` + `Space` → 清除全部記錄（`,,RS` 只重置字頻，不動查字歷史）
- 記錄會隨字頻一起透過 freq.json 同步（macOS syncFolder／iOS iCloud）
...
- 字體大小、透明度、模式提示大小
- **高對比** — 候選字加粗，提升可讀性
- **Debug 模式** — 記錄操作日誌到 `debug.log`，供除錯使用
- **蝦頭方向** — 狀態列圖示方向
...
放在 `~/Library/Application Support/Yabomish/tables/` 下，格式為 tab-separated：
