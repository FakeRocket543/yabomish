...
| **3) 極簡安裝** | 無聯想、無詞庫，僅打字＋查碼＋繁簡轉換＋字頻排序（`-DMINIMAL` 編譯） | ~2MB |

> **語料下載說明**：語料二進位檔（`*.bin`）**不隨 git repo 散佈**。全新 clone 執行 `yabomish.sh` 時會自動從 GitHub Release 下載對應等級的語料包（完整約 100MB／精簡約 15MB，SHA-256 驗證）；下載失敗（離線或 Release 尚未發佈）不影響安裝，App 會在首次打字時自動重試。「極簡」僅原始碼安裝可選，不隨 Release DMG 發佈。打字、查碼、繁簡轉換不需要語料，離線亦可用。
