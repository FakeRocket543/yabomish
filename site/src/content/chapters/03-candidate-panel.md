...
可在設定程式 → 外觀分頁開啟「**橫向**」選項，將選字窗切換為水平排列（類似固定模式的佈局，但跟隨游標）。

特性：

- 選字窗自動跟隨文字游標位置移動
- 支援**垂直**或**水平**排列，在設定程式中切換
- **方向鍵自動適配**：
  - 垂直模式：↑↓ 移動候選、←→ 翻頁
  - 水平模式：←→ 移動候選、↑↓ 翻頁
- **螢幕邊界偵測**：
...
`Tab` 一律下一頁（`Shift+Tab` 也是下一頁——沒有「上一頁」快捷鍵）。方向鍵的行為依排列模式自動適配（見 3.1）：

| 操作 | 垂直排列（游標模式） | 水平排列／固定模式 |
|------|--------------------|-------------------|
| 翻頁 | `←` ／ `→` | `↑` ／ `↓` |
| 移動候選 | `↑` ／ `↓` | `←` ／ `→` |

候選字超過一頁時，選字窗會顯示頁碼（如 `1/3`）。
