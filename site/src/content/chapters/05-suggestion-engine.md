...
- **直接打碼** 忽略聯想，繼續輸入
- 按 **Esc** 關閉聯想候選列表
- 純聯想顯示（組字已空）時，**Enter 與空白鍵都不會代選**——Enter 收提示並把換行還給 app、空白鍵輸出空白後收提示
- 預設**不預先反白**任何聯想候選（偏好舊觀感可在設定→輸入開啟「聯想預先反白」）；未反白時方向鍵從第一個候選開始導航

聯想功能可在設定程式的「輸入」頁整體開啟或關閉（也可用 `,,SG` 快速切換）。

---

## 三層架構

聯想引擎由三層文字資料來源組成（另有第四張 Emoji 聯想卡，可調序、可關閉），每一層獨立運作，結果合併後依排序呈現：
...
| 資訊工程   | 資訊、電機、電子、通訊、航太、核能、輕工業等 |
| 自然科學   | 數學、物理、化學、動物生態、植物、魚類、地科等 |
| 地理軍事   | 地理、測量、軍事、海洋等              |
...
除了選擇預設策略，你也可以直接拖拉三層的順序卡片，自訂任意排列。卡片還包含第四張「Emoji 聯想」：排最前時 Emoji 一定看得到（預設），移到其他位置則文字聯想優先、聯想列有空位時才顯示 Emoji，點擊卡片可完全關閉。
...
Emoji 聯想資料來自 Unicode CLDR，由語料檔 `emoji_char_map.json` 驅動（隨安裝或首次語料下載提供，非擴充表）。

在設定程式的「聯想與詞庫」頁可以調整 Emoji 聯想的位置，或將它關閉：

- **排最前（預設）**：Emoji 固定出現在聯想列開頭，最多 3 個
- **排其他位置**：文字聯想優先，聯想列（10 格）還有空位時才顯示 Emoji
- **點擊卡片關閉**：聯想列完全不建議 Emoji
...
> 的、了、在、是、和、與、或、而、但、也、都、就、被、把、讓、給、從、到、對、為、著、過、嗎、呢、吧、啊、喔、哦、啦
